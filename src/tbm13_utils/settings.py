from .encoding import Serializable, SerializableFile
...
_settings: SerializableFile[str, RawSetting] = SerializableFile(
    "config.json", "key", RawSetting
)
...
class Setting[T]:
    ...
    def value(self, value: T | None):
        ...
        if type(value) is not self.value_type:
            raise TypeError(f"Expected {self.value_type}, got {type(value)}", self.key)

        _settings.set(RawSetting(key=self.key, value=str(value)))
