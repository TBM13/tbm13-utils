...
from .console import BaseValidator, color_input, color_print, remove_style
from .typing import inherits_args1

__all__ = [
    "BaseTextFile",
    "MultiKeySerializableFile",
    "Serializable",
    "SerializableFile",
    "StringSerializable",
    "StringSerializableFile",
    "StringSerializableValidator",
    "UpdateMode",
    "base64_decode",
    "base64_encode",
    "field_extra_config",
...
class UpdateMode(Enum):
    ...
    the other object's when it's not the default.
    """
    LIST_APPEND = 1
    """For fields of type `list`. Appends to this list the
    values of the other object's list.
    """
    LIST_APPEND_IF_NOT_EXISTS = 2
    """For fields of type `list`. Appends to this list the
    values of the other object's list that don't already exist.
    """
    MERGE = 3
    ...
    * For `dict` and `set`, merges the other object's value.
    ...
    def validate_type(self, field_type: type[Any], other_type: type[Any]):
        if (
            self == UpdateMode.LIST_APPEND
            or self == UpdateMode.LIST_APPEND_IF_NOT_EXISTS
        ):
            ...
            field_valid = field_type in (dict, set) or issubclass(
                field_type, Serializable
            )
            other_valid = other_type in (dict, set) or issubclass(
                other_type, Serializable
            )

            if not (field_valid and other_valid):
                raise TypeError(
                    "UpdateMode.MERGE can only be used with fields of type"
                    " dict, set or Serializable",
                    field_type,
                    other_type,
                )
...
def field_extra_config(
    ...
    """Builds a dictionary to customize the behavior of a field in a `Serializable`.

    Pass the returned dictionary as the `json_schema_extra`
    parameter of a `Field` or `PrivateAttr`
    """
    return {
        "update_mode": update_mode,
        "print": print,
        "print_key": print_key,
        "print_key_style": print_key_style,
        "print_value_style": print_value_style,
    }


class Serializable(BaseModel, ABC):  # noqa: PLW1641
    ...
    You can treat this as a normal (but strict) Pydantic `BaseModel`.
    You can customize it as you wish (e.g., by adding a `model_config`).

    Suggestions:
    * To customize the behavior of a field on `update` or `print` methods,
       define it with `Field` or `PrivateAttr` and build its
       `json_schema_extra` using `field_extra_config`.
    * Override `get_printable_title` to customize the title used by `print()`.
    """
    ...
    def update(self, other: Self):
        ...
        * Fields will be updated according to their `UpdateMode`.
        """
        for key in other.model_dump(exclude_unset=True):
            ...
            if update_mode == UpdateMode.REPLACE:
                ...
                setattr(self, key, val)  # Ensure we trigger pydantic's validation
    ...
    def print(self, other: "Serializable|None" = None, spaces: int = 0):
        ...
        If `other` isn't `None`, ALL the differences between this and `other`
        ...
        for var_name, line in lines.items():
            ...
            elif (isinstance(value, list) and isinstance(other_value, list)) or (
                isinstance(value, tuple) and isinstance(other_value, tuple)
            ):
                open = "["
                close = "]"
...
class StringSerializableValidator[T: StringSerializable](BaseValidator[T]):
    ...
    def _validate(self, text: str):
        try:
            ...
            raise ValidationError(message=str(e), cursor_position=len(text)) from e

    @override
    def _parse(self, user_input: str) -> T:
        return self.type.from_string(user_input)
...
class StringSerializable(BaseModel, ABC):
    ...
    * Mark fields as final.
    ...
    def input(cls, *args: Any, **kwargs: Any) -> Self:
        return StringSerializableValidator(cls).input(*args, **kwargs)
...
class BaseTextFile(ABC):
    ...
    * The file is locked with `filelock` while reading/writing to avoid race conditions.
    ...
    def _write(self):
        ...
        with self._lock, open(self.path, "w", encoding="utf8") as file:
    ...
    def modified_externally(self) -> bool:
        ...
        return (
            self._last_size != stats.st_size or self._last_mtime_ns != stats.st_mtime_ns
        )
    ...
    * Before every read & write the file is re-read if it was modified externally.
    ...
    def _deserialize(self, lines: list[str]) -> None:
        ...
        for line in lines:
            ...
            self._values.add(self._value_type.from_string(line))

    @override
    def _serialize(self) -> str:
        return "\n".join(val.to_string() for val in self._values)
...
class SerializableFile[K, V: Serializable](BaseTextFile):
    """A dictionary-like object that contains `Serializable` objects of type `V`
    as values.
    ...
    * Before every read & write the file is re-read if it was modified externally.
    ...
    def __init__(self, path: str, key_name: str, value_type: type[V]):
        ...
        if key_name not in self._value_type.model_fields:
            raise ValueError(
                f"'{key_name}' is not a valid field for {self._value_type}"
            )
    ...
    @override
    def _serialize(self) -> str:
        return "\n".join(obj.model_dump_json() for obj in self._dic.values())
        ...
        * `old` and `new` don't have the same key.
        ...
        with self.modify():
            ...
            if old != self._dic[key]:
                raise ValueError(
                    "Given value doesn't match the stored value", old, self._dic[key]
                )
    ...
    * Before every read & write the file is re-read if it was modified externally.
    ...
    def _deserialize(self, lines: list[str]) -> None:
        ...
        for line in lines:
            ...
            if obj in list:
                raise ValueError(
                    f'"{self.path}" has a duplicate object for key', key, obj
                )
    ...
    def _get_key(self, obj: V | list[V], validate: bool) -> K:
        if isinstance(obj, list):
            ...
            if validate and not all(getattr(o, self._key_name) == key for o in obj):
